...
# 1. Update Key Visual section steps to match Slide 10 precisely
old_kv_steps_start = content.find('{[')
# Let's find the array of 4 items inside section id="criacao"
criacao_idx = content.find('id="criacao"')
if criacao_idx != -1:
    sub_content = content[criacao_idx:criacao_idx+2000]
    print("Found criacao section snippet")

channels_replacement = '''        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {/* Instagram */}
...
# Find digital section in content
dig_start = content.find('id="digital"')
if dig_start != -1:
    # find the grid div after digital title
    grid_start = content.find('grid grid-cols-1', dig_start)
    if grid_start != -1:
        # find where the next section starts or end of grid
        # Let's replace the grid of cards
        print("Found digital grid start at", grid_start)
