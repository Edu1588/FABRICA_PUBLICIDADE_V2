...
# Locate digital section
dig_idx = content.find('id="digital"')
if dig_idx != -1:
    # Find the grid container after digital title
    grid_target = content.find('grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3', dig_idx)
    if grid_target == -1:
        grid_target = content.find('grid grid-cols-1', dig_idx)
    
    if grid_target != -1:
        # Find the closing of this grid (next section or financial partner banks)
        banks_idx = content.find('{/* Financial Partner Banks */', grid_target)
        if banks_idx != -1:
            new_grid = '''        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
          {/* Instagram */}
        ...
        '''
            content = content[:grid_target] + new_grid + content[banks_idx:]
            with open('src/components/FabricaAzulLandingPage.tsx', 'w') as f:
                f.write(content)
            print("Successfully updated digital channels grid with official logos and background images!")
